...
import { Modal, Form, Input, InputNumber, Switch, Button, Select, Mentions, Typography } from "antd";
import { getMetrics } from "../../services/metricService";

const { Option } = Select;
const { Text } = Typography;

const FormulaModal = ({ visible, onClose, onSubmit, initialValues, loading }) => {
        ...
        >
            <Form form={form} layout="vertical">
                ...
                >
                    ...
                    >
                        {metrics.map((m) => (
                            <Option key={m.id} value={m.metricCode} label={`${m.metricCode} ${m.metricName}`}>
                                <div className="flex justify-between">
                                    <span className="font-semibold">{m.metricCode}</span>
                                    <span className="text-gray-500 truncate ml-2" style={{ maxWidth: 200 }}>
                                        {m.metricName}
                                    </span>
                                </div>
                            </Option>
                ...
                <Form.Item
                    ...
                    extra={
                        <span className="text-xs text-gray-400">
                            Gõ ký tự <Text code>@</Text> để gợi ý các chỉ số. Hỗ trợ các toán tử: +, -, *, /
                        </span>
                    }
                >
                    <Mentions
