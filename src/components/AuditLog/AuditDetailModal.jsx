...
const AuditDetailModal = ({ visible, onClose, record }) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: "oldVal",
            key: "oldVal",
            render: (text) => text === null || text === undefined ? <Text type="secondary" italic>(Trống)</Text> : text,
            width: "35%"
        },
        {
            title: "Giá trị mới",
            ...
            key: "newVal",
            render: (text, row) => (
                <Text type={row.isChanged ? "success" : "default"} strong={row.isChanged}>
                    {text === null || text === undefined ? <Text type="secondary" italic>(Đã xóa)</Text> : text}
                </Text>
            ),
            width: "35%"
        }
    ...
    return (
        ...
        >
            <div className="mb-4">
                <p><strong>Người thực hiện:</strong> {record?.username}</p>
                <p><strong>Thời gian:</strong> {record?.timestamp ? dayjs(record.timestamp).format("DD/MM/YYYY HH:mm:ss") : ""}</p>
                <p><strong>Đối tượng:</strong> {record?.description}</p>
            </div>
